...
object PlanTaskScheduler {
    ...
    fun freeIntervalsForDate(
        ...
        return freeIntervalsFromBusyRanges(unavailableByDay[dayOfWeek].orEmpty() + extraBusyRanges)
    }

    /**
     * 이미 날짜별로 정확히 계산된 불가 시간 목록(예: plan-generation 이 NICE 시간표로 계산한
     * 그날의 하교시각 기반 범위)으로 그날의 빈 시간을 계산한다.
     */
